...
func (ic *ImageWriter) CommitAttestationsManifest(ctx context.Context, opts *ImageCommitOpts, p exptypes.Platform, target string, stmts []intoto.Statement, refType string) (*ocispecs.Descriptor, error) {
	var (
		manifestType = ocispecs.MediaTypeImageManifest
		configType   = ocispecs.MediaTypeImageConfig
	)
	...
	layers := make([]ocispecs.Descriptor, len(stmts))
	for i, st := range stmts {
		env, err := sign.SignInTotoStatement(ctx, st, sign.GithubActionsOIDC)
		if err != nil {
